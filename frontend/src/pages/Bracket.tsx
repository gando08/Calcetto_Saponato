...
export function Bracket() {
  ...
  const bracketQuery = useQuery({
    queryKey: ["bracket", tid, gender],
    queryFn: () => tournamentApi.getBracket(tid, gender),
    enabled: Boolean(tid)
  });
  ...
  const generateMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["bracket", tid, gender] });
    }
  });

  const advanceMutation = useMutation({
    mutationFn: ({ matchId, winnerTeamId }: { matchId: string; winnerTeamId: string }) =>
  ...
  const matches = useMemo(
    () => ((((bracketQuery.data || {}) as { matches?: BracketMatch[] }).matches || []).slice() as BracketMatch[]),
    [bracketQuery.data]
  );
  ...
  const onAdvance = async (match: BracketMatch, winnerTeamId: string | null | undefined) => {
    if (!tid || !winnerTeamId || !match.id) return;
    setErrorMessage(null);
    try {
      await advanceMutation.mutateAsync({ matchId: match.id, winnerTeamId });
    } catch (e: unknown) {
      setErrorMessage(e instanceof Error ? e.message : "Errore avanzamento bracket.");
    }
  };
    ...
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-600">Genera il tabellone e promuovi il vincitore di ogni partita al turno successivo.</p>
        </div>
      </header>
      ...
      <section className="bg-white p-4 rounded shadow flex flex-wrap gap-3 items-end">
        ...
        >
          {generateMutation.isPending ? "Generazione..." : "Rigenera Bracket"}
        </button>

        <button
          className="px-3 py-2 border rounded"
          type="button"
          onClick={() => void queryClient.invalidateQueries({ queryKey: ["bracket", tid, gender] })}
          disabled={!tid}
        >
          Refresh
        </button>
      ...
      <section className="bg-white p-4 rounded shadow space-y-4">
        ...
        {bracketQuery.isLoading ? (
          ...
          <div className="text-sm text-slate-500">Nessun bracket disponibile. Clicca "Rigenera Bracket".</div>
        ) : (
          <div className="space-y-4">
                ...
                <div key={roundBlock.round} className="border rounded p-3 bg-slate-50">
                  ...
                  <div className="space-y-2">
                    {roundBlock.matches.map((match) => (
                      <div key={match.id || `${match.round}-${match.bracket_position}-${match.phase}`} className="rounded border bg-white p-2 text-sm">
                        <div className="text-xs text-slate-500 mb-1">
                          {phaseToLabel(match.phase)} • Pos. {match.bracket_position + 1} • {match.status || "pending"}
                        </div>
                        <div className="space-y-1">
                          <button
                            className="w-full text-left px-2 py-1 rounded border"
                            type="button"
                            onClick={() => void onAdvance(match, match.team_home_id)}
                            disabled={!match.team_home_id || !match.id || advanceMutation.isPending}
                          >
                            {match.placeholder_home}
                          </button>
                          <div className="text-slate-600 text-center">vs</div>
                          <button
                            className="w-full text-left px-2 py-1 rounded border"
                            type="button"
                            onClick={() => void onAdvance(match, match.team_away_id)}
                            disabled={!match.team_away_id || !match.id || advanceMutation.isPending}
                          >
